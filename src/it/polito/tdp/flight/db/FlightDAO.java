...
public class FlightDAO {
	...
	public Map<Integer, Airline> getAllAirlines() {
		...
		this.compagnie = new TreeMap<Integer, Airline> () ;
...
//		List<Airline> list = new ArrayList<>();
		try {
			...
			while (res.next()) {
//				list.add(
					Airline temp = new Airline(res.getInt("Airline_ID"), res.getString("Name"), res.getString("Alias"),
						res.getString("IATA"), res.getString("ICAO"), res.getString("Callsign"),
						res.getString("Country"), res.getString("Active"));
					compagnie.put(temp.getAirlineId(), temp);
					}
			conn.close();
			return compagnie;
...
//		List<Airport> list = new ArrayList<>();
		this.tuttiAereoporti = new TreeMap <Integer, Airport> ()  ;
		try {
			Connection conn = DBConnect.getConnection();
			PreparedStatement st = conn.prepareStatement(sql);
			ResultSet res = st.executeQuery();
...
//				list.add(
					...
					tuttiAereoporti.put(temp.getAirportId(), temp) ;
			}
			conn.close();
